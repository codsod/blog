...
https://github.com/codsod/blog/blob/master/%E6%AF%8F%E6%97%A5%E9%80%89%E5%81%9A/Binary%20Tree.py
### 2.py中的ASCII码
t = "A"
# 将'A'变成'B'
t = chr(ord(t) + 1) if t != 'Z' else 'A'
在这个例子中，我们使用了ord()函数获取字符的ASCII码，然后加1，最后用chr()函数将新的ASCII码转换回字符。对于字母'Z'的情况，我们进行了特殊处理使其循环回到'A'。
### 3.简单的整数划分
有难度的dp
```py
...
```
### 4.二维列表的初始化
原始代码中使用了 c=[[0]*n]*n ，这样的做法会导致所有行共享同一内存地址，也就是说改变其中一行的元素会影响到其他行。

修正后的代码 c = [[0] * n for _ in range(n)] 是正确初始化二维列表的方式，这种方式会创建n个独立的一维列表
## day15 2024.4.4
### 1.append与extend的区别
### 2.字典排序的写法
datanum=sorted(datanum.items(),key=lambda x:x[0],reverse=True)
注意返回的是一个列表
### 3.输入进来的值一定要明确变量类型
## day16 2024.4.12
### 1.dict查找
利用dict实现，查找是O(1) 列表O(n)
## day17 2024.4.15
### 1.{}的集合用法
siw-{word1}
## day18 2024.4.23
...
## day19 2024.5.26
### 1.单调栈
结合下标的顺序性而产生的栈，先进先出且有单调性
### 2.dijkstra算法
要注意元组的排序方法，node和dist要把dist放在前面，因为dist是元组中的第一个元素，所以要放在前面，然后按照dist进行排序